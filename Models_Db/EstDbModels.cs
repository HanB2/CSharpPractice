...
namespace web_esm.Models_Db
{
	public class EstDbModels : DatabaseConnection
	{
		...
		//RELEASE_CODE 콤보박스  이 부분 estCode 받아서 처리되게끔 해야 한다.
		public List<schTypeArray> GetReleaseCodeSelectBox(EstIframeCostModels getModel, string estCode)
		{
		...
		public EstShippingFee GetEstIframeCostList(EstIframeCostModels getModel, string estCode)
		{
			...
			if (!String.IsNullOrEmpty(getModel.station))  //STATION
				listQuery += " AND EST_CODE = " + getModel.station.Trim();

			if (!String.IsNullOrEmpty(getModel.nation))  //도착국가
				listQuery += " AND NATION_CODE = " + getModel.nation.Trim();

			if (!String.IsNullOrEmpty(getModel.type))  //출고타입
				listQuery += " AND NATION_CODE = " + getModel.type.Trim();
			...
			if (listDt != null && listDt.Rows.Count != 0)
			{
				...
				model.WEIGHT = double.Parse(listDt.Rows[0]["WEIGHT"].ToString().Trim());
				model.SHIPPING_FEE_NOR = double.Parse(listDt.Rows[0]["SHIPPING_FEE_NOR"].ToString().Trim());
				model.SHIPPING_FEE_STC = double.Parse(listDt.Rows[0]["SHIPPING_FEE_STC"].ToString().Trim());
		...
		public string SetEstGrade(EstGradeViewModel model)
		{
			//세션에 저장된 ESE코드 가져오기
			//HttpContext context = HttpContext.Current;
			//model.Item.EST_CODE = context.Session["EST_CODE"].ToString();
			...
			{
				//ese_group 그룹 
				exeQueryStr = " INSERT INTO est_group ( GROUP_ID , GROUP_NAME, EST_CODE )VALUES(  ";
