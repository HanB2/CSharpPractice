...
namespace web_ese.Models_Db
{
	public class StocDbModels : DatabaseConnection
	{
			...
			using (ExcelPackage excelPackage = new ExcelPackage())
			{
				...
				if (listDt != null && listDt.Rows.Count > 0)
				{
					for (int i = 0; i < listDt.Rows.Count; i++)
					{
						...
						int CATEGORY1 = int.Parse(listDt.Rows[i]["CATEGORY1"].ToString().Trim());
						int CATEGORY2 = int.Parse(listDt.Rows[i]["CATEGORY2"].ToString().Trim());
						int CATEGORY3 = int.Parse(listDt.Rows[i]["CATEGORY3"].ToString().Trim());
						int CATEGORY4 = int.Parse(listDt.Rows[i]["CATEGORY4"].ToString().Trim());

						string CATE_NAME = "";
						ProdDbModels prodDb = new ProdDbModels();
						if (CATEGORY1 > 0)
							CATE_NAME += prodDb.GetCategoryName(CATEGORY1);

						if (CATEGORY2 > 0)
							CATE_NAME += " > " + prodDb.GetCategoryName(CATEGORY2);

						if (CATEGORY3 > 0)
							CATE_NAME += " > " + prodDb.GetCategoryName(CATEGORY3);

						if (CATEGORY4 > 0)
							CATE_NAME += " > " + prodDb.GetCategoryName(CATEGORY4);

						workSheet.Cells[row, col++].Value = CATE_NAME;
		...
		public StocInOutModels GetStocInOut(StocInOutModels model)
		{
			...
			if (!String.IsNullOrEmpty(model.schStat))      
		...
		public StocInOutModels StocInOutView(StocInOutModels model)
		{
			...
			string getNationCol = cf.GetFieldName("NATIONNAME");
			string getCateCol = cf.GetFieldName("CATENNAME");
