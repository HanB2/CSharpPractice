...
namespace web_ese.Models_Db
{
	public class EtsDbModels : DatabaseConnection
	{
		...
		public string EtsReqIns(EtsReqModels model)
		{
			...
			model.Item.WAYBILLNO = "000000004"; //중복 인서트가 안될 것이므로 테스트할때 + 1 씩 해서 작업할것
			...
			if (model.act_type != null && model.act_type == "ins")
			{
				...
				{
					exeQueryArray.Add(CreatOrdGoods(model.Item.EST_CODE, model.Item.ESE_CODE, model.Item.WAYBILLNO, model.Item.ORDERTYPE, item, out errorStr));
					//바코드 중복검사
					if (errorStr != "")
						return errorStr;
				}
			...
			else if (model.act_type != null && model.act_type == "updt")
			{
				...
				foreach (OrdGoods item in model.Items)
				{
					...
					if (errorStr != "")
					{
						result = "바코드 정보가 없습니다. [" + item.BARCODE + "] ";
						return result;
					}
				}
			}
		...
		public string CreatStat(string EST_CODE, string ESE_CODE, string WAYBILLNO)
		{
			...
			string resultStr = " INSERT INTO ord_status (EST_CODE, ESE_CODE, UPLOADYMD, WAYBILLNO, STATUS ) VALUES ( ";
			resultStr += "  '" + EST_CODE + "'";
			resultStr += " , '" + ESE_CODE + "'";
			resultStr += " , DATE_FORMAT(NOW(), '%Y%m%e') ";
			resultStr += " , '" + WAYBILLNO + "'";
			resultStr += " , 10 ";	//입고대기 상태값 10 
			resultStr += " ) ";
		...
		//배송 상품 등록 쿼리 생성
		public string CreatOrdGoods(string EST_CODE, string ESE_CODE, string WAYBILLNO, int type, OrdGoods item, out string errorStr)
		{
			errorStr = "";	//바코드 중복 체크 용
			...
			if (type == 1)
			{
				string selectQueryStr = "SELECT SKU, UNIT_WEIGHT, WEIGHT_UNIT FROM stc_goods WHERE ESE_CODE = '" + ESE_CODE + "' AND '" + item.BARCODE + "'";
				...
				{
					errorStr = "바코드 정보가 없습니다. [" + item.BARCODE + "] ";
					return "";
				}
		...
		{
			CommFunction cf = new CommFunction();
			string getNationCol = cf.GetFieldName("NATIONNAME");
